...
aantal_metingen = 5  # Variable die het aantal meetingen aangeeft
...
scale = qwiic.start_scale()  # De qwiicscale wordt gestart en opgeslagen als het "scale" object
app = Flask(__name__)  # Maak een object aan van de flask module om de webserver aan te spreken

calibration_factor, nul_gewicht = qwiic.calibrate_scale(scale)  # kalibreert het rekstrookje
sensor_data = qwiic.start_lijst(aantal_metingen)  # maak een lijst aan voor de sensordata
time_stamp_list = qwiic.start_lijst(aantal_metingen)  # maak een lijst aan voor de timestamps
...
def show_main_page():
    ...
    sensor_data, time_stamp_list = qwiic.meting(scale, calibration_factor, nul_gewicht, sensor_data, time_stamp_list, aantal_metingen)
...
# deze functie roep het qwiic script aan om de lijsten te legen (momenteel niet geïmplementeerd in qwiic.py)
@app.route('/clear')
def clear():
    qwiic.clear()
    return show_main_page()