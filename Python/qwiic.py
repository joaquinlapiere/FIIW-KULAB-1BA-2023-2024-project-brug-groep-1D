"""
Dit bestand is een bibliotheek met alle functies die nodig zijn bij het uitmeten van een rekstrookje die bevestigd is op
...
rekstrookmeting vergemakkelijken in andere bestanden.
"""

import qwiicscale  #De qwiicscale bibliotheek wordt geïmporteerd om de rekstrookmetingen mogelijk te maken.
import time  #De time bibliotheek wordt gebruikt om timestamps van de meeting toe te kunnen voegen

Average_Amount: int = 500  #variabele die bepaald hoeveel waarders gebruiktworden om een gemiddelde te nemen


def start_scale():

    """:
    De start_scale() functie gaat of het qwiic bordje juist functioneerd en juist communiceert.
    Deze functie moet altijd als eerste uitgevoerd worden voor de andere functies in dit document gaan werken.

    :returns: None: Bij het mislopen van het opstarten wordt er niets gereturned
...
def getAverage(scale, averageAmount):
    ...
    """
    De getAverage() functie neemt een gemiddelde van een een N aantal meetwaardes van de rekstrook. De waarde van N wordt
    ...
    :return: total: Dit is de gemiddelde waarde die de funtie berekend heeft.
    """

    total = 0

    for i in range(0, averageAmount):  #Deze loop zal N aantal keer doorlopen worden (N is bepaald door averageAmount)
        total += scale.getReading()    #Een meetwaarde die verkregen wordt via de getReading() fucntie uit de
...
def nul_waarde(scale):
    ...
    rekstrookje te kalibreren en metingen uit te voeren die meteen omgrekend worden naar een massa in gram.
...
def calibrate_scale(scale):
    ...
    """
    De callibrate_scale() is een functie die het rekstrookje zal callibreren. eerst zal er een meetwaarde genomen worden
    als er geen kracht op het aluminium plaatje geplaatst wordt en er dus ook 0 rek is. Na het ingeven van de waarde van
    een massa en na het plaatsen van de zelfde massa op het plaatje zal er een nieuwe meting genomen worden. Met de 2
    meetwaarders en met de gekende massa wordt een "callibration factor" bekomen. deze heeft de betekenis dat elke keer
    dat de bekomen meetwaarde met de waarde van de "callibration factor" stijgt, de geplaatste massa met 1 gram gestegen
    is.

    :param scale: => Dit argument is het "scale" object dat nodig is voor de meeting uit te kunnen voeren.
    :return: callibration_factor: Deze waarde wordt in de read_massa() functie gebruikt om uit een meetwaarde de masse
                                  te kunnen bepalen.
             nul_massa: deze waarde is de meetwaarde wanneer er geen massa op het alluminium plaatje staat en wordt ook
                        in de read_massa() funtie gebruikt.
    """
...
def read_massa(scale, calibration_factor, nul_massa):
    ...
    :return: massa: Dit is de gementen massa.
    """
...
def start_lijst(aantal_metingen):
    ...
    """
    Deze functie maakt een lege lijst aan die 5 keer de lengte heeft van de globale variabele "aantam_meetingen" (deze
...
def meting(scale, calibration_factor, nul_massa, metingen, time_stamp_list, aantal_metingen, time_interval=None):
    ...
    """
    De meting() funtie zal eerste de geimporteerde lijsten "metingen" en "time_stamp_list" opschuiven. dan zal ze een
